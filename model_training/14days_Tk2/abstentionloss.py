...
class NotWrongLoss(tf.keras.losses.Loss):
    ...
    * The y_pred must be the output from a softmax, so that y_pred sums to 1.
    ...
    def call(self, y_true, y_pred):
        ...
        self.updater.update(y_pred)
        #tf.print("Called update in loss — alpha:", self.updater.alpha)
        return tf.reduce_mean(loss, axis=-1)


##modifying the NotWrongLoss to include class_weights 
# -----------------------------------------------------------------------------
class ClassWeight_NotWrongLoss(tf.keras.losses.Loss):
    """Abstention loss function that highlights being "not wrong", rather than
     being right, while penalizing abstentions.
    ...
    * The y_pred must be the output from a softmax, so that y_pred sums to 1.
    ...
    def call(self, y_true, y_pred):
        q = tf.cast(1 - y_pred[:, -1], tf.float64)
        r = tf.cast(tf.boolean_mask(y_pred, y_true), tf.float64)
        s = tf.cast(y_pred[:, -1], tf.float64)

        true_class = tf.cast(tf.argmax(y_true, axis=1), tf.int32)
        class_weights = tf.constant([1.0, 1.8, 0.4], dtype=tf.float64)
        sample_weights = tf.gather(class_weights, true_class)

        def spinup_loss():
            return tf.math.log(q) - tf.math.log(r)

        def abstain_loss():
            return -tf.math.log(r + s) - self.updater.alpha * tf.math.log(q)

        base_loss = tf.cond(
            self.updater.current_epoch < self.spinup_epochs,
            spinup_loss,
            abstain_loss
        )
...
class Washington(AlphaUpdater):
    ...
    * https://github.com/thulas/dac-label-noise/blob/master/dac_loss.py
    ...
    def __init__(
        ...
        self.kp = tf.constant(kp, dtype=tf.float64)  # PID proportional coefficient
        self.ki = tf.constant(ki, dtype=tf.float64)  # PID integral coefficient
        self.kd = tf.constant(kd, dtype=tf.float64)  # PID derivative coefficient
        self.dt = tf.constant(dt, dtype=tf.float64)  # PID pseudo time increment
        self.mu = tf.constant(mu, dtype=tf.float64)  # smoothing factor
        self.clip_at = tf.constant(clip_at, dtype=tf.float64)  # clipping limit for control

        self.smoothed = tf.Variable(0.0, trainable=False, dtype=tf.float64)
        self.integral = tf.Variable(0.0, trainable=False, dtype=tf.float64)
    ...
    def update(self, y_pred):
        ...
        zero_frac = tf.cast(tf.math.zero_fraction(voted), tf.float64)
        pv = self.mu * self.smoothed + (1.0 - self.mu) * zero_frac
        ...
        self.alpha.assign(tf.math.maximum(tf.constant(0.0, dtype=tf.float64), self.alpha - control))
